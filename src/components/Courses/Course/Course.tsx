...
const Course: FC<CourseProps> = ({ studentTask }) => {
  ...
  const updateTeacher = async () => {
    ...
    const res = await fetch(`${allJson}/instructors/${studentTask.instructor_id}`);
    const instructor = await res.json();

    const updatedInstructor = {
      ...instructor,
      pending_reviews: [...instructor.pending_reviews, newTask]
    };

    await fetch(`${allJson}/instructors/${studentTask.instructor_id}`, {
      method: 'PUT',
